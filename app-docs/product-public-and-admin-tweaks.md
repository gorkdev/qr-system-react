...
### 1.4. QR okunabilirliği kapalıyken davranış (site ayarı)

Bu davranış, backend’deki `site_settings.qr_enabled` alanına bağlıdır (detaylar için aşağıdaki backend bölümüne bak).

- QR okunabilirlik ayarı **kapalıysa** ve ürün bulunmuşsa:
  - 5 saniyelik overlay sonrası ürün detayı yerine sade bir “hizmet dışı” ekran gösterilir.
  - İçerik:
    - Üstte küçük bir dil anahtarı: `TR` ve `EN` butonları (tek tıklama ile dil değişir).
    - Seçili dile göre mesaj:
      - TR: Başlık **“Şu anda hizmet dışı”**, açıklama *“QR kod üzerinden ürün detayları geçici olarak kullanıma kapatılmıştır. Lütfen daha sonra tekrar deneyin.”*
      - EN: Başlık **“Currently unavailable”**, açıklama *“Product details via QR codes are temporarily disabled. Please try again later.”*
  - Yerleşim:
    - `min-h-[60vh]` yüksekliğinde, hem yatay hem dikey merkezde (`items-center justify-center text-center`), bordersız minimal bir blok.
- QR kapalıyken **ziyaret kaydı** oluşturulmaz:
  - `ProductPublic` içindeki ziyaret kaydı `useEffect`’i `!qrEnabled` durumunda erken return eder.
